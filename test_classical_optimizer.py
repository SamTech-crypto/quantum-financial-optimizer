...
class TestClassicalOptimizer(unittest.TestCase):
    def setUp(self):
        ...
        self.optimizer = ClassicalPortfolioOptimizer(self.returns, self.cov_matrix, risk_free_rate=0.01, max_var=100_000)
    ...
    def test_zero_returns(self):
        optimizer = ClassicalPortfolioOptimizer(np.zeros(3), self.cov_matrix, risk_free_rate=0.01)
        weights = optimizer.optimize()
        self.assertAlmostEqual(sum(weights), 1.0, places=5)

    def test_single_asset(self):
        optimizer = ClassicalPortfolioOptimizer(np.array([0.01]), np.array([[0.0001]]), risk_free_rate=0.01)
        weights = optimizer.optimize()
